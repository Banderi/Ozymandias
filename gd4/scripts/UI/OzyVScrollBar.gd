...
@export var value = 0 : set = update_scrollbar_from_value_PRE_TREE
...
func update_scrollbar_from_grabber():
	value = int((grabber.position.y - 26) / float(size.y - 78) * (max_value - min_value)) + min_value
	update_and_emit()
func update_and_emit():
	...
	if max_value == min_value: # can not scroll -- 
		...
		grabber.position.y = 26 + (size.y - 78) / float(max_value - min_value) * (value - min_value)
		grabber.position.x = 5.5
...
func _on_BtnGrabber_gui_input(_event): # continuous grabber dragging event
	if grabber.pressed:
		grabber.position.y = clamp(get_local_mouse_position().y + grabber_mouse_click_delta, 26, size.y - 52)
		update_scrollbar_from_grabber()
func _on_BtnGrabber_pressed(): # on initial click/grab
	grabber_mouse_click_delta = grabber.position.y - get_local_mouse_position().y
