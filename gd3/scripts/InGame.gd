...
onready var camera_zoom_target = 5.0
...
func _process(delta):
	if Game.STATE == Game.States.Ingame:
		...
		# camera zoom
		CAMERA.zoom = Viewports.clampdamp(CAMERA.zoom, Vector2.ONE * camera_zoom_target, 30.0, delta)
		...
		if camera_previous_game_coords != null:
			...
			if Input.is_mouse_button_pressed(MOUSE_DRAG_CONT):
				...
				CAMERA.position += delta_pos * camera_zoom_target * 0.25
		...
		CAMERA.position.y = clamp(CAMERA.position.y, 0, 7000)
